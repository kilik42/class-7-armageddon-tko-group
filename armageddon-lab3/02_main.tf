...
resource "aws_route" "armageddon_shinjuku_to_sp_route01" {
  route_table_id         = aws_route_table.private_route.id
  destination_cidr_block = "10.80.0.0/16" # Sao Paulo VPC CIDR (students supply)
...
# RDS Security Group change in Tokyo (allows São Paulo)
resource "aws_vpc_security_group_ingress_rule" "rds_lab_mysql_from_sao" {
  security_group_id = aws_security_group.rds_lab_sg.id
...
resource "aws_vpc_security_group_ingress_rule" "rds_lab_mysql_from_ec2_private" {
  security_group_id            = aws_security_group.rds_lab_sg.id
