...
#HTTP 443 for alb
resource "aws_vpc_security_group_ingress_rule" "alb_ingress_443_from_cf" {
  security_group_id = aws_security_group.alb_sg.id
  prefix_list_id    = data.aws_ec2_managed_prefix_list.armageddon_cf_origin_facing01.id
  from_port         = 443
  to_port           = 443
  ip_protocol       = "tcp"
}

resource "aws_vpc_security_group_egress_rule" "alb_to_ec2_private_http" {
  security_group_id            = aws_security_group.alb_sg.id
  referenced_security_group_id = aws_security_group.ec2_private_sg.id # destination = the EC2 private SG
  from_port                    = 80
  to_port                      = 80
  ip_protocol                  = "tcp"
}
