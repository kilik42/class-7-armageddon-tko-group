...
#HTTP 443 for alb
resource "aws_security_group_rule" "alb_ingress_http_cf" {
  type = "ingress"
  security_group_id = aws_security_group.alb_sg.id
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
  prefix_list_ids   = [data.aws_ec2_managed_prefix_list.armageddon_cf_origin_facing01.id]
}
...
resource "aws_security_group_rule" "alb_to_ec2_http" {
  ...
  source_security_group_id = aws_security_group.ec2pri_sg.id
}
